...
use crate::windows::releases::{CheckError, Release, outcome, select, select_latest, supersedes};
...
#[test]
fn check_results_use_the_running_applications_version() {
    let published = undated("v2.0.0");

    assert_eq!(
        outcome(Ok(Some(published.clone())), "v2.0.0"),
        Status::UpToDate
    );
    assert_eq!(
        outcome(Ok(Some(published.clone())), "v1.0.0"),
