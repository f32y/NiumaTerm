//! Terminal key encoding shared by rio frontends (rioterm and rio-engine/NiumaTerm).
//!
//! `build_key_sequence` turns one keystroke into the PTY byte sequence the shell
...
bitflags! {
    ...
    pub struct KeyEncodeFlags: u16 {
        ...
        const REPORT_ALL_KEYS_AS_ESC = 1 << 5;
        const REPORT_ASSOCIATED_TEXT = 1 << 6;
    }
}
...
pub fn build_key_sequence(key: &KeyInput, mods: ModifiersState, flags: KeyEncodeFlags) -> Vec<u8> {
    ...
    let kitty_seq = flags.intersects(
        KeyEncodeFlags::REPORT_ALL_KEYS_AS_ESC
            | KeyEncodeFlags::DISAMBIGUATE_ESC_CODES
            | KeyEncodeFlags::REPORT_EVENT_TYPES,
    );
    ...
    let associated_text = text.filter(|text| {
        flags.contains(KeyEncodeFlags::REPORT_ASSOCIATED_TEXT)
...
/// Encode a key **press** into terminal bytes, or `None` when the key should be
/// delivered as plain text by the caller (printable input). This is the unified
/// entry point: it folds together the named-key escapes that rioterm previously kept
/// as static `Action::Esc` binding rows (app-cursor SS3 arrows/Home/End, F1–F4 SS3,
/// Backspace→DEL, Shift+Tab) and the general [`build_key_sequence`] encoder (kitty +
...
/// Frontend-facing terminal input policy: named keys go through the terminal
/// encoder, printable presses fall back to the frontend-provided text, and key
/// releases are sent only when kitty event-type reporting is enabled.
...
/// Wrap paste payloads for DEC bracketed paste mode.
pub fn bracket_paste(body: &[u8], bracketed: bool) -> Vec<u8> {
    if !bracketed {
        return body.to_vec();
    }
...
fn named_key_escape(
    ...
    // Backspace → DEL. Plain allows DISAMBIGUATE; shift/alt do not (matches the rows).
    ...
    // Shift+Tab / Shift+Alt+Tab → CSI Z, not kitty.
    if named == NamedKey::Tab && !all_keys && !disamb {
...
fn should_build_sequence(
    ...
    let disambiguate = flags.contains(KeyEncodeFlags::DISAMBIGUATE_ESC_CODES)
...
impl SequenceBuilder {
    ...
    fn try_build_textual(
        ...
        if character.chars().count() == 1 {
            ...
            // yet.
            let payload = if self.flags.contains(KeyEncodeFlags::REPORT_ALTERNATE_KEYS)
