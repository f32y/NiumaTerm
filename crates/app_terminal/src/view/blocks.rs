...
use crate::view::mouse::block_gutter_hit;
use crate::view::*;
...
impl TerminalPane {
    ...
    fn block_list_total_px(
        ...
        let frozen: f32 = store
            .items()
            .iter()
            .map(|item| block_list::item_px(item, cols, cell_h, pad_rows))
            .sum();
    ...
    pub(crate) fn record_frozen_chrome(
        &mut self,
    ...
    pub(crate) fn block_list_point_at(
        &self,
    ...
    fn jump_to_frozen_item(&mut self, direction: i8, cx: &mut Context<Self>) {
        ...
        let Some(target) =
            block_list::nav_item_top(&store, cols, cell.height_px, pad_rows, resolved, direction)
        else {
            return;
        };
