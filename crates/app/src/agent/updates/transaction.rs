...
use crate::agent::updates::AgentUpdates;
use crate::agent::{AgentPane, RecoveryReadiness, RecoverySnapshot, RestorationReadiness};
use crate::ui::Shell;
use crate::window::ShellRegistry;

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(in crate::agent::updates) enum UpdateMode {
    WhenIdle,
    StopNow,
}

impl UpdateMode {
    pub(in crate::agent::updates) fn interrupts_active_work(self) -> bool {
        self == Self::StopNow
    }
}

pub(in crate::agent::updates) enum PreflightResolution {
    Ready(Vec<RecoverySnapshot>),
    Wait,
    Failed(String),
}

pub(in crate::agent::updates) fn resolve_preflight(
...
pub(in crate::agent::updates) fn combine_transaction_error(
...
pub(in crate::agent::updates) fn affected_installation_indices(
