...
pub(super) use crate::agent::session::update_recovery::UpdateSuspension;
pub(crate) use crate::agent::session::update_recovery::{
    RecoveryReadiness, RecoverySnapshot, RestorationReadiness,
};
use crate::agent::transcript::LAST_RESPONSE_LIMIT;
use crate::agent::*;
...
pub(in crate::agent) fn directories_match(left: Option<&str>, right: Option<&str>) -> bool {
...
impl AgentPane {
    ...
    pub(in crate::agent) fn push_item(&mut self, item: SessionItem, cx: &mut Context<Self>) {
        self.push_item_with_images(item, Vec::new(), cx);
    }

    /// Append an item along with the images it carried, which only a sent
    /// user message has.
    pub(in crate::agent) fn push_item_with_images(
        &mut self,
        item: SessionItem,
    ...
    pub(in crate::agent) fn handle_question_control(
        &mut self,
