...
/// hour".
pub(in crate::agent) const LAST_RESPONSE_LIMIT: Duration = Duration::from_secs(60 * 60);

pub(in crate::agent) fn should_show_jump_to_latest(
...
pub(in crate::agent) fn working_label(
    started: Instant,
...
/// changing altogether.
pub(in crate::agent) fn last_response_label(seconds: u64) -> String {
...
pub(in crate::agent) fn is_work_row(item: &SessionItem) -> bool {
    matches!(
        item,
...
pub(in crate::agent) fn hidden(item: &SessionItem) -> bool {
    match item {
...
pub(in crate::agent) fn truncated_user_prompt(text: &str) -> Option<&str> {
...
pub(in crate::agent) fn fenced_code_block_as(output: &str, lang: &str) -> String {
...
pub(in crate::agent) fn detect_output_language(output: &str) -> &'static str {
...
pub(in crate::agent) fn strip_read_gutter(output: &str) -> Option<String> {
...
pub(in crate::agent) fn file_extension_lang(path: &str) -> String {
    Path::new(path)
        .extension()
...
pub(in crate::agent) fn command_execution_heading(purpose: Option<&str>) -> &str {
    purpose
        .filter(|purpose| !purpose.trim().is_empty())
        .unwrap_or_else(|| i18n("agent-transcript-run-command"))
}

pub(in crate::agent) fn command_execution_detail(
    command: &str,
...
pub(in crate::agent) fn compaction_label(detail: &Compaction) -> &'static str {
...
pub(in crate::agent) fn compaction_row_is_expandable(kind: AgentKind) -> bool {
    kind.caps().expandable_compaction_rows
}

pub(in crate::agent) fn compaction_trigger_label(trigger: CompactionTrigger) -> &'static str {
    match trigger {
...
pub(in crate::agent) fn compaction_accounting(detail: &Compaction) -> Vec<String> {
...
pub(in crate::agent) fn relative_time(at: SystemTime) -> String {
...
pub(in crate::agent) fn compact_token_count(tokens: u64) -> String {
    match tokens {
...
pub(in crate::agent) fn permission_icon(value: Option<&str>) -> IconName {
    match value {
