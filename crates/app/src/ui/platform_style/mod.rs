//! Platform conventions of the application chrome.
//!
...
//! [`PlatformStyle`], which `MacOs` and `Windows` implement. Both implementations compile on every host, so editing one
//! platform's appearance is type-checked from the other; [`Host`] names the
//! implementation this build targets and is what views read.
//!
...
//!
//! The `app` library has a counterpart of this module for the conversation
//! surfaces it draws. Each crate styles the views it owns, because the library
//! cannot see this executable's chrome types.
...
use crate::ui::platform_style::windows::Windows;
