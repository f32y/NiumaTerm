...
pub(in crate::agent_pane) const MAX_ATTACHMENTS: usize = 8;
...
impl Attachment {
    pub(in crate::agent_pane) fn bytes(&self) -> &[u8] {
        self.image.bytes()
    }

    pub(in crate::agent_pane) fn format(&self) -> ImageFormat {
        self.image.format()
    }

    pub(in crate::agent_pane) fn placeholder(&self) -> &str {
        &self.placeholder
    }

    /// The renderable form of this attachment. Encoded bytes render directly,
    /// so a thumbnail costs no file on disk.
    pub(in crate::agent_pane) fn image(&self) -> Arc<Image> {
        self.image.clone()
    }
}

/// Why a paste produced no attachment.
pub(in crate::agent_pane) enum AttachError {
...
#[derive(Default)]
pub(in crate::agent_pane) struct PendingAttachments {
    items: Vec<Attachment>,
}

impl PendingAttachments {
    pub(in crate::agent_pane) fn is_empty(&self) -> bool {
        self.items.is_empty()
    }

    pub(in crate::agent_pane) fn iter(&self) -> impl Iterator<Item = &Attachment> {
        self.items.iter()
    }

    pub(in crate::agent_pane) fn clear(&mut self) {
        self.items.clear();
    }

    /// Decode a clipboard image, shrink it past the edge cap, and attach it.
    /// Returns the placeholder to insert at the caret.
    pub(in crate::agent_pane) fn attach(&mut self, image: &Image) -> Result<String, AttachError> {
    ...
    pub(in crate::agent_pane) fn placeholder_at(&self, index: usize) -> Option<&str> {
    ...
    pub(in crate::agent_pane) fn reconcile(&mut self, text: &str) -> Option<String> {
...
pub(in crate::agent_pane) fn placeholder_text(number: usize) -> String {
...
pub(in crate::agent_pane) fn scratch_dir(route: &str) -> PathBuf {
