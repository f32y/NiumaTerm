...
pub(crate) struct TranscriptView {
    pub(in crate::agent_pane) items: Vec<Entry>,
    ...
    pub(in crate::agent_pane) transcript_list: ListState,
    pub(in crate::agent_pane) row_specs: Vec<RowSpec>,
    ...
    pub(in crate::agent_pane) expanded_groups: HashSet<usize>,
    ...
    pub(in crate::agent_pane) expanded_turns: HashSet<u64>,
    /// Work-log rows whose detail (command output, reasoning text) is
    /// expanded, keyed by transcript index.
    pub(in crate::agent_pane) expanded_rows: HashSet<usize>,
    ...
    pub(in crate::agent_pane) completed_turn_output_tokens: HashMap<u64, u64>,
    ...
    pub(in crate::agent_pane) interrupted_turns: HashSet<u64>,
    ...
    pub(in crate::agent_pane) compacting: bool,
    ...
    pub(in crate::agent_pane) cwd: Option<String>,
    pub(in crate::agent_pane) kind: AgentKind,
...
impl TranscriptView {
    ...
    pub(in crate::agent_pane) fn push_stamped(&mut self, turn: u64, item: SessionItem) {
        self.items.push(Entry {
    ...
    pub(in crate::agent_pane) fn contains_item(&self, id: &str) -> bool {
        self.items.iter().any(|entry| entry.item.id() == Some(id))
    }

    /// Fold an authoritative completed payload into the entry that streamed it.
    pub(in crate::agent_pane) fn merge_completed(&mut self, item: &SessionItem) {
    ...
    pub(in crate::agent_pane) fn latest_agent_message(&self, turn: u64) -> Option<&str> {
    ...
    pub(in crate::agent_pane) fn is_compacting(&self) -> bool {
        self.compacting
    }

    pub(in crate::agent_pane) fn set_compacting(
        &mut self,
        compacting: bool,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(in crate::agent_pane) fn was_interrupted(&self, turn: u64) -> bool {
        self.interrupted_turns.contains(&turn)
    }

    pub(in crate::agent_pane) fn mark_interrupted(&mut self, turn: u64) {
        self.interrupted_turns.insert(turn);
    }
