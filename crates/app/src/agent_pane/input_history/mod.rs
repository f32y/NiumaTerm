...
use crate::agent_pane::input_history::store::{
...
pub(in crate::agent_pane) enum InputHistoryDirection {
    Older,
    Newer,
}
...
#[derive(Default)]
pub(in crate::agent_pane) struct InputHistoryNavigation {
    entries: Arc<[String]>,
    index: Option<usize>,
}

impl InputHistoryNavigation {
    pub(in crate::agent_pane) fn reset(&mut self) {
...
impl AgentPane {
    pub(in crate::agent_pane) fn handle_input_history_control(
        &mut self,
        direction: InputHistoryDirection,
        window: &mut Window,
        cx: &mut Context<Self>,
