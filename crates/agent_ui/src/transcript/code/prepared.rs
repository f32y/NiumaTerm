...
use crate::transcript::strip_read_gutter;
...
pub(in crate::transcript) const VIRTUAL_TRANSCRIPT_MAX_SEGMENT_BYTES: usize = 4 * 1024;

pub(in crate::transcript) fn should_virtualize_transcript(text: &str) -> bool {
    text.len() >= VIRTUAL_TRANSCRIPT_MIN_BYTES
        || text.lines().take(VIRTUAL_TRANSCRIPT_MIN_ROWS + 1).count() > VIRTUAL_TRANSCRIPT_MIN_ROWS
}

pub(in crate::transcript) fn transcript_segments(text: &str) -> Vec<Range<usize>> {
