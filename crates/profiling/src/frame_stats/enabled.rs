...
//!
//! Compiled only with `enable_profiling`. Collection is off until
//! [`set_enabled`] turns it on. Hot paths check [`enabled`] before timing events.
//!
...
/// Clock state used by timed frame-statistics hooks.
pub type Timer = Instant;

/// Start a clock only while runtime collection is enabled.
#[inline]
