...
use crate::transcript::reveal::{Disclosures, RevealKey, RevealedPart, revealed_block};
...
pub(super) fn render_compaction_row(
    ...
    let expandable = compaction_row_is_expandable(kind);
    ...
    v_flex()
        ...
        .child(
            agent_card().child(header).children(
                expanded.then(|| {
                    render_compaction_detail(cwd, disclosures, index, &detail, window, cx)
                }),
            ),
        )
        .into_any_element()
