...
#[test]
fn disk_images_report_failed_writes_and_reuse_paths() {
    ...
    assert!(write_attachments(images.into_iter(), &scratch.0).is_err());

    fs::remove_dir(scratch.0.join("image-2.png")).unwrap();

    let paths = write_attachments(images.into_iter(), &scratch.0).unwrap();

    assert_eq!(
        paths,
        [
            scratch.0.join("image-1.png"),
            scratch.0.join("image-2.png"),
            scratch.0.join("image-3.png")
        ]
    );
    assert_eq!(fs::read(&paths[0]).unwrap(), [1]);
    assert_eq!(fs::read(&paths[1]).unwrap(), [2]);
    assert_eq!(fs::read(&paths[2]).unwrap(), [3]);
...
#[test]
fn empty_images_do_not_create_a_directory_and_unusable_scratch_returns_an_error() {
    let scratch = Scratch::new();
    let unused = scratch.0.join("unused");

    assert!(
        write_attachments(iter::empty(), &unused)
            .unwrap()
            .is_empty()
    );
