...
///
/// Each terminal has two screens (main and alt). Each screen owns its own
/// image cache, placements, and number
...
pub struct Graphics {
    ...
    /// Memory limit for graphics storage. The 320 MiB default bounds decoded
    /// pixel memory without making normal image-heavy sessions churn.
    ...
    ///
    /// When a client places an image without an explicit `p=` (or with `p=0`),
    /// allocate a unique placement id internally so multiple placements of the
...
impl Graphics {
    ...
    pub fn take_queues(&mut self) -> Option<UpdateQueues> {
        ...
        if remove_queue.is_empty() && self.pending.is_empty() && self.pending_images.is_empty() {
            return None;
        }
    ...
    ///
    /// Eviction preserves visible content as long as possible:
    ...
    pub fn evict_images(
        ...
        debug!(
            "Graphics memory: need to evict {} bytes (current: {}, limit: {}, required: {})",
    ...
    pub fn register_placed_texture(&mut self, graphic_id: GraphicId, weak: Weak<TextureRef>) {
...
fn test_graphics_eviction_fails_when_not_enough_space() {
    ...
    // The first image is the only candidate, so it is evicted even while in use.
