...
use crate::graphics::{ColorType, GraphicData, GraphicId, ResizeCommand, ResizeParameter};
...
pub struct KittyGraphicsState {
    ...
    /// Counter for auto-assigned image IDs. Commands without an explicit `i=` or
    /// `I=` still need an internal storage key. We allocate from the high half
...
impl KittyGraphicsState {
    ...
    fn allocate_image_id(&mut self) -> u32 {
        ...
        self.next_auto_image_id = self.next_auto_image_id.checked_add(1).unwrap_or(0x80000000);
        id
    }
}
...
pub struct KittyGraphicsCommand {
    ...
    /// not supply `i=` or `I=`. Suppressing responses for these commands avoids
    /// exposing an internal-only id to the client.
    implicit_id: bool,
...
/// this returns `None` because the client could not correlate an unaddressed response.
fn encode_response(
    image_id: u32,
    image_number: u32,
    placement_id: u32,
...
pub fn parse(params: &[&[u8]], state: &mut KittyGraphicsState) -> Option<KittyGraphicsResponse> {
    ...
    // `i=` and `I=` are mutually exclusive because one image cannot be addressed
    // by both id and number without ambiguity.
    ...
    // A query needs an image id. Without one we cannot build an addressable response, so we
    ...
    // and we allocate one to obtain a stable internal storage key.
    //
    ...
    let image_key = if cmd.image_id > 0 || cmd.image_number > 0 {
        ...
        // implicit so the internal id is not echoed to the client.
    ...
    debug!(
        "Kitty graphics action: {:?}, format={:?}, width={}, height={}, image_id={}, payload_len={}",
        cmd.action,
        cmd.format,
        cmd.width,
        cmd.height,
    ...
    match cmd.action {
        Action::Transmit | Action::TransmitAndDisplay => {
            debug!(
                "Creating graphic data: format={:?}, medium={:?}, compression={:?}, width={}, height={}, payload_len={}",
                cmd.format,
                cmd.medium,
                cmd.compression,
                cmd.width,
        ...
        Action::Frame | Action::Animate | Action::Compose => {
            // Animation actions are not supported. Return an explicit error so
            // clients can detect the lack of support and
    ...
    for pair in control_data.split(',') {
        if let Some((key, value)) = pair.split_once('=') {
            match key {
                ...
                "c" => match cmd.action {
                    Action::Frame | Action::Compose => cmd.base_frame = value.parse().unwrap_or(0),
                ...
                "z" => match cmd.action {
                    Action::Frame | Action::Animate => cmd.frame_gap = value.parse().unwrap_or(0),
                ...
                // Delete
                "d" => cmd.delete_action = value.as_bytes().first().copied().unwrap_or(b'a'),
...
/// no-padding variant because intermediate Kitty chunks omit padding.
///
...
fn create_graphic_data(cmd: &KittyGraphicsCommand) -> Result<GraphicData, GraphicError> {
    ...
    if cmd.format != Format::Png && (cmd.width > MAX_DIMENSION || cmd.height > MAX_DIMENSION) {
        debug!(
        ...
        TransmissionMedium::SharedMemory => {
            ...
            #[cfg(windows)]
            {
                ...
                use windows_sys::Win32::System::Memory::{
                    FILE_MAP_READ, MEMORY_BASIC_INFORMATION, MapViewOfFile, OpenFileMappingW,
    ...
    match cmd.format {
        ...
        Format::Gray | Format::GrayAlpha | Format::Rgb24 | Format::Rgba32 => {
            ...
            // Validate data size
            let expected_size = cmd.width as usize * cmd.height as usize * bytes_per_pixel;
            ...
            let (pixels, is_opaque) = match cmd.format {
                Format::Gray => {
                    ...
                    let mut rgba = Vec::with_capacity(cmd.width as usize * cmd.height as usize * 4);
                    for &g in &pixel_data {
...
mod tests {
    ...
    fn parse_kitty_graphics_protocol(keys: &str, payload: &str) -> Option<KittyGraphicsResponse> {
    ...
    fn test_animation_frame_returns_unsupported_error() {
        // a=f (transmit animation frame) is not implemented; surfacing
        // EINVAL:unsupported action lets clients fall back.
        let payload = "AAAA";
    ...
    fn test_incomplete_image_accumulation() {
        ...
        // Each chunk uses a multiple of 4 base64 chars (aligned on 3-byte binary
        // boundaries), with padding only on
        ...
        let result1 = parse(&params1, &mut state)
            .expect("intermediate chunks must return a `pending_chunk` response, not None");
    ...
    fn test_incomplete_image_accumulation_padded_chunks() {
        ...
        let r1 =
            parse(&params1, &mut state).expect("padded intermediate chunk must not return None");
    ...
    fn test_chunked_matches_single_shot() {
        ...
        let single = parse_kitty_graphics_protocol("a=t,f=32,s=1,v=1,i=301", single_pixel_b64)
    ...
    fn test_chunked_preserves_first_chunk_metadata() {
        ...
        let response = parse(&p2, &mut state).expect("final chunk must produce a response");
    ...
    fn test_chunked_with_zlib_compression() {
        ...
        let response = parse(&p2, &mut state).expect("final chunk must parse and decompress");
    ...
    fn test_query_requires_image_id() {
        ...
        let result = parse_kitty_graphics_protocol("a=q", "").expect("response struct must exist");
        assert!(result.response.is_none());

        // Query with only I= set (no i=): we surface the EINVAL
        // addressed by image_number, so the client can correlate.
        let result =
            parse_kitty_graphics_protocol("a=q,I=7", "").expect("response struct must exist");
        let body = result.response.expect("error message expected");
        assert!(
        ...
        let result =
            parse_kitty_graphics_protocol("a=q,i=42", "").expect("response struct must exist");
    ...
    fn parse_delete(keys: &str) -> DeleteRequest {
        let resp = parse_kitty_graphics_protocol(keys, "").expect("delete must produce a response");
        resp.delete_request
    ...
    fn test_security_checks() {
        ...
        for path in &["/proc/self/environ", "/sys/class/net", "/dev/null"] {
            ...
            let response = parse_kitty_graphics_protocol("a=t,t=f,f=32,s=1,v=1,i=1", &encoded)
    ...
    fn test_quiet_mode() {
        ...
        // q=2 requests absolute silence, including errors.
