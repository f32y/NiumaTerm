...
/// Fixed-size VT parameter list with optional subparameters. The Sixel DCS
/// [`Parser::new`] reads its Ps2 background selector from this minimal surface,
/// avoiding a dependency on a second VT parser.
#[derive(Default)]
pub struct Params {
    /// Subparameter count for each parameter, indexed alongside `params`.
    subparams: [u8; MAX_PARAMS],
    /// All parameters and subparameters, flattened.
...
impl Params {
    ...
    /// Iterate the parameters, each yielded as its subparameter slice.
    #[inline]
...
/// Immutable subparameter iterator over [`Params`].
...
///
/// Multiple Sixel implementations assume this palette, so using it preserves
/// cross-terminal rendering consistency.
