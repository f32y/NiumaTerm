...
Run DeepSeek Harness as an Agent Tab against the local interface its `dsh web` host serves, so a user who installs the harness themselves can prompt it, watch it work, approve what it asks for, and stop it.
