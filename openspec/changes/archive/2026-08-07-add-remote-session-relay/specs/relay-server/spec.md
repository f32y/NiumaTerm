...
Public relay server implemented as a Cloudflare Worker and Durable Object (TypeScript in the repository's `relay/` directory). Each `host_id` maps to one DO instance through `idFromName(host_id)`. WebSocket hibernation reduces idle cost. The connection model follows paseo v2: one Host control socket plus one Host data socket per Client connection, so the relay treats application frames as opaque bytes.
