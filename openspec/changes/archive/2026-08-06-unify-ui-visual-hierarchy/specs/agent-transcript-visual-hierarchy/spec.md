...
The visual hierarchy change SHALL preserve transcript virtualization, selectable user and assistant text, context-menu copying, tail-follow behavior, jump-to-latest behavior, keyboard composer actions, and expansion state keyed to existing transcript entries and turns.
...
- **THEN** those interactions retain their current behavior while the new visual hierarchy is applied
