...
use super::mask_pattern::{MaskPattern, normalize_number_input};
use super::mode::InputMode;
use super::number_input::{NumberStep, StepAction};
use super::{DisplayMap, MASK_CHAR, number_input};
...
use crate::input::popovers::{ContextMenu, DiagnosticPopover, HoverPopover};
use crate::input::search::SearchPanel;
use crate::input::{HoverDefinition, InlineCompletion, Lsp, Position, RopeExt as _, Selection};
...
pub struct InputState {
    ...
    /// Render a static (non-blinking) block caret instead of the blinking beam.
    pub(super) cursor_block: bool,
...
impl InputState {
        ...
        let _subscriptions = vec![
            ...
            cx.observe_window_activation(window, |input, window, cx| {
                if window.is_window_active() {
                    ...
                    // A static block caret never blinks: skip the 500 ms blink
                    // loop entirely so an idle focused input demands no frames.
                    if focus_handle.is_focused(window) && !input.cursor_block {
    ...
    /// Make the input grow its height with the content (wrapped rows) between `min_rows`
    /// and `max_rows`.
    ///
    /// On a [`InputMode::CodeEditor`] (set via [`Self::code_editor`]) this keeps the
    /// editor's syntax highlighting and just switches its height to auto-grow; otherwise
    /// it selects the plain [`InputMode::AutoGrow`] mode.
    pub fn auto_grow(mut self, min_rows: usize, max_rows: usize) -> Self {
        if self.mode.is_code_editor() {
            self.mode.set_code_editor_auto_grow(min_rows, max_rows);
        } else {
    ...
    /// Render a static (non-blinking) block caret sized to the line height,
    /// instead of the default blinking beam. Useful for terminal-style prompts.
    pub fn cursor_block(mut self, block: bool) -> Self {
    ...
    pub fn focus(&self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        if self.cursor_block {
            // Static block caret: no blink loop (saves idle frame demand).
            return;
        }
    ...
    pub(crate) fn show_cursor(&self, window: &Window, cx: &App) -> bool {
        (self.focus_handle.is_focused(window) || self.is_context_menu_open(cx))
            && !self.disabled
            // Block caret is static; the beam caret blinks.
            && (self.cursor_block || self.blink_cursor.read(cx).visible())
    ...
    pub(super) fn pause_blink_cursor(&mut self, cx: &mut Context<Self>) {
        if self.cursor_block {
            // Static block caret: pause would restart the blink loop.
            return;
        }
        self.blink_cursor.update(cx, |cursor, cx| {
            cursor.pause(cx);
