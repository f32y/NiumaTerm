...
// Plain (id, generation) value; bindgen doesn't derive comparisons for it, but
// downstream event types (`HarvestEvent`) carry handles by value and compare.
impl PartialEq for bindings::BlockHandle {
    fn eq(&self, other: &Self) -> bool {
        self.id == other.id && self.generation == other.generation
    }
}
impl Eq for bindings::BlockHandle {}
impl std::hash::Hash for bindings::BlockHandle {
