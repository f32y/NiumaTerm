...
pub(crate) struct DirectManipulationHandler {
    ...
    /// True from touchpad contact until the viewport returns to READY. While
    /// set, the demand-driven frame pump must keep polling `update()`.
...
impl DirectManipulationHandler {
    ...
    pub fn on_pointer_hit_test(&self, wparam: WPARAM) {
        unsafe {
            ...
            if GetPointerType(pointer_id, &mut pointer_type).is_ok() && pointer_type == PT_TOUCHPAD
            {
                ...
                // Status callbacks only fire from `update()` polling, so mark
                // the interaction here to start the polling frames.
    ...
    /// True while a touchpad interaction (including inertia) is in flight and
    /// per-frame `update()` polling must continue.
    pub fn is_interacting(&self) -> bool {
        self.interacting.get()
    }
}
...
impl IDirectManipulationViewportEventHandler_Impl for DirectManipulationEventHandler_Impl {
    fn OnViewportStatusChanged(
        ...
        // Drive the demand-driven pump: keep polling frames while the
        // viewport is running or in inertia; stop once it is READY.
        self.interacting.set(current != DIRECTMANIPULATION_READY);
