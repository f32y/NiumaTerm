...
pub struct WindowsPlatform {
    ...
    /// Whether the UI-critical threads run at above-normal priority. Applied
    /// to the main thread on toggle and to the vsync thread at spawn/toggle.
    prioritize_ui_threads: Cell<bool>,
    /// The `VSyncProvider` frame-pacing thread, kept so its priority can be
    /// adjusted after it has spawned.
    vsync_thread: RefCell<Option<std::thread::JoinHandle<()>>>,
}
...
impl WindowsPlatform {
    pub fn new(headless: bool) -> Result<Self> {
        ...
        let frame_pump = Arc::new(FramePump::new());
    ...
    /// Raise (`true`) or restore (`false`) the priority of the UI-critical
    /// threads: the calling thread — this must be called on the main (UI)
    /// thread — and the `VSyncProvider` frame-pacing thread. The choice
    /// sticks, so a vsync thread spawned later starts at the right priority.
    pub fn set_ui_thread_priority(&self, above_normal: bool) {
        self.prioritize_ui_threads.set(above_normal);
        set_thread_priority(unsafe { GetCurrentThread() }, above_normal);
    ...
    fn generate_creation_info(&self) -> WindowCreationInfo {
        WindowCreationInfo {
            ...
            frame_pump: self.frame_pump.clone(),
        }
    }
    ...
    fn begin_vsync_thread(&self) {
        ...
        let frame_pump = self.frame_pump.clone();
        ...
        let thread = std::thread::Builder::new()
            ...
            .spawn(move || {
                ...
                loop {
                    // Demand-driven pump: park with zero CPU until any window
                    // arms a frame request, then run at vsync cadence while
                    // demand persists.
                    if !frame_pump.any_armed() {
                        frame_pump.park();
                        continue;
                    }
            ...
            .unwrap();
        if self.prioritize_ui_threads.get() {
            set_thread_priority(HANDLE(thread.as_raw_handle()), true);
        }
        *self.vsync_thread.borrow_mut() = Some(thread);
    }
}

/// Set one thread's priority to above-normal or back to normal.
fn set_thread_priority(thread: HANDLE, above_normal: bool) {
    let priority = if above_normal {
        THREAD_PRIORITY_ABOVE_NORMAL
    } else {
        THREAD_PRIORITY_NORMAL
    };
    unsafe {
        SetThreadPriority(thread, priority).log_err();
    }
}
...
impl WindowsPlatformInner {
    fn new(context: &mut PlatformWindowCreateContext) -> Result<Rc<Self>> {
        ...
        Ok(Rc::new(Self {
            ...
            frame_pump: context.frame_pump.clone(),
            dispatcher: context
                .dispatcher
                .as_ref()
    ...
    fn close_one_window(&self, target_window: HWND) -> bool {
        self.frame_pump.unregister(target_window);
...
pub(crate) struct WindowCreationInfo {
    ...
    pub(crate) frame_pump: Arc<FramePump>,
}
...
fn file_open_dialog(
    ...
    unsafe {
        ...
        let (_filter_names, _filter_patterns, filter_specs) =
            file_dialog_filter_specs(&options.file_types);
        if options.files && !filter_specs.is_empty() {
...
fn file_dialog_filter_specs(
...
) -> (Vec<HSTRING>, Vec<HSTRING>, Vec<Common::COMDLG_FILTERSPEC>) {
    ...
    for filter in filters {
        ...
        let name: &str = &filter.name;
        names.push(HSTRING::from(name));
        patterns.push(HSTRING::from(
            filter
                .extensions
                .iter()
                .map(|extension| file_dialog_extension_pattern(extension))
                .join(";"),
        ));
    }
    ...
    (names, patterns, specs)
}

fn file_dialog_extension_pattern(extension: &str) -> String {
