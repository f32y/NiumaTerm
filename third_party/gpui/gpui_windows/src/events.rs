...
impl WindowsWindowInner {
    ...
    fn handle_size_change(
            ...
            && let Err(e) = self.state.renderer.borrow_mut().resize(device_size)
        {
            ...
            // The recovery runs on the vsync thread's awake path; make sure a
            // parked pump wakes to perform it.
    ...
    fn handle_dm_pointer_hit_test(&self, wparam: WPARAM) -> Option<isize> {
        ...
        // DirectManipulation only surfaces events via per-frame polling in
        // `draw_window`, so start the frames now.
        self.state.frame_pump.arm(&self.state.frame_request);
        None
    }
    ...
    fn draw_window(&self, handle: HWND, force_render: bool) -> Option<isize> {
        ...
        // Sustain polling frames for the whole touchpad interaction
        // (including inertia); the flag drops when the viewport is READY.
        if self.state.direct_manipulation.is_interacting() {
            self.state.frame_pump.arm(&self.state.frame_request);
        }
