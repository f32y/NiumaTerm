...
impl DirectXRenderer {
    ...
    /// Clear the render target for the applied background appearance and encode every
    ...
    pub(crate) fn render_to_image(
        ...
        self.set_background_appearance(background_appearance)?;
        self.render(scene)?;
        ...
        for blur in blurs {
            ...
            for (index, pass_params) in passes.into_iter().enumerate() {
                ...
                unsafe {
                    ...
                    context.RSSetViewports(Some(slice::from_ref(&targets.viewport)));
                }
            ...
            unsafe {
                context
                    .OMSetRenderTargets(Some(slice::from_ref(&resources.render_target_view)), None);
                context.RSSetViewports(Some(slice::from_ref(&resources.viewport)));
            }
...
mod tests {
    ...
    use anyhow::Result;
    use gpui::{
        BackdropBlur, Bounds, ContentMask, DevicePixels, Quad, ScaledPixels, Scene,
        WindowBackgroundAppearance, black, point, red, size, white,
    };
    use gpui_util::ResultExt;
    ...
    #[test]
    fn backdrop_blur_preserves_alpha_and_restores_the_viewport() -> Result<()> {
        let window = TestWindow::new()?;
        let devices = DirectXDevices::new()?;
        let mut renderer = DirectXRenderer::new(window.0, &devices, false)?;
        renderer.resize(size(DevicePixels(64), DevicePixels(64)))?;
        ...
        for alpha in [1., 0.5] {
            ...
            for (x, color) in [(0., black()), (32., white())] {
                scene.insert_primitive(Quad {
                    ...
                    content_mask,
                    background: color.opacity(alpha).into(),
                    ..Default::default()
                });
            }
            ...
            let image_pixel = |x, y| {
                image.get_pixel_checked(x, y).ok_or_else(|| {
                    anyhow::anyhow!("pixel ({x}, {y}) is outside the rendered image")
                })
            };
            let before_blur = image_pixel(28, 4)?;
            let blurred = image_pixel(28, 32)?;
            let expected_alpha = (alpha * 255.).round() as u8;
            assert_eq!(before_blur.0[0], 0);
            assert!(blurred.0[0] > 10 && blurred.0[0] < expected_alpha);
            assert!(blurred.0[3].abs_diff(expected_alpha) <= 1);
            assert_eq!(image_pixel(58, 58)?.0, [255, 0, 0, 255]);
        }
        Ok(())
    }
