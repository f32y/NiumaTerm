...
mod tests {
    use crate::{
        FileDialogFilter, PathPromptOptions, SystemNotification, SystemNotificationAction,
    ...
    #[gpui::test]
    async fn test_path_prompt_preserves_file_type_filters(cx: &mut TestAppContext) {
        let receiver = cx.update(|cx| {
            cx.prompt_for_paths(PathPromptOptions {
                files: true,
                directories: false,
        ...
        cx.simulate_path_prompt_response(|options| {
            assert_eq!(options.file_types.len(), 1);
            assert_eq!(options.file_types[0].name, "Executables");
            assert_eq!(options.file_types[0].extensions, vec!["exe"]);
            Some(vec![PathBuf::from("pwsh.exe")])
        });

        let response = receiver.await.unwrap().unwrap();
        assert_eq!(response, Some(vec![PathBuf::from("pwsh.exe")]));
    }
}
