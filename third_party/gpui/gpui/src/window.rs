...
use crate::{
    ...
    WindowOptions, WindowParams, WindowTextSystem, frame_stats, point, prelude::*, px, rems, size,
    transparent_black,
};
...
impl Window {
    ...
    pub fn draw(&mut self, cx: &mut App) -> ArenaClearNeeded {
        ...
        if let Some(draw_started_at) = draw_started_at {
            frame_stats::record_draw(draw_started_at.elapsed(), redrawn_views);
        }
        #[cfg(feature = "profiler")]
        {
    ...
    fn present(&mut self) {
        ...
        if let Some(present_started_at) = present_started_at {
...
mod tests {
    ...
    #[gpui::test]
    fn test_reentrant_frame_request_rearms_frame_source(cx: &mut TestAppContext) {
        struct RequestsFrameOnPaint(crate::TestWindow);

        impl Render for RequestsFrameOnPaint {
            fn render(&mut self, _: &mut Window, _: &mut Context<Self>) -> impl IntoElement {
                let test_window = self.0.clone();
                canvas(
                    |_, _, _| {},
                    move |_, _, _, _| {
                        let baseline = test_window.frame_wake_count();
        ...
        let test_window = cx.test_window(window.into());
        test_window.simulate_frame_request(RequestFrameOptions::default());
        cx.add_window(|_, _| RequestsFrameOnPaint(test_window.clone()));
        test_window.simulate_frame_request(RequestFrameOptions::default());
    }
