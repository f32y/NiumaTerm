            ...
            BaseTooltip::new("tooltip-popup")
                ...
                .map(|this| {
                    this.child(div().map(|this| match self.content {
                        TooltipContext::Text(ref text) => this.max_w(px(420.)).child(text.clone()),
        ...
        .on_hover({
            ...
            move |hovered, window, cx| {
                if let Some(overlay) = Root::tooltip_overlay(window, cx) {
                    if *hovered {
                        // Explicit side placement stays attached to the control.
                        // Automatic tooltips follow the pointer and clear its glyph.
                        let bounds = if preferred_placement.is_some() {
                            trigger_bounds_cell.get()
                        } else {
                            Bounds::new(window.mouse_position(), gpui::size(px(0.), px(18.)))
                        };
                        overlay.update(cx, |o: &mut BaseTooltipOverlay, cx| {
                            ...
                            let request = match preferred_placement {
                                ...
                                None => request.align(gpui_base::Align::Start),
                            };
