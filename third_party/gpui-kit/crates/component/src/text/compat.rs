...
impl TextView {
    ...
    /// Creates a text view that displays markup literally.
    pub fn plain(id: impl Into<ElementId>, text: impl Into<SharedString>) -> Self {
        let id = id.into();
        Self {
            id: id.clone(),
            inner: gpui_base::TextView::plain(id, text),
            text_style: None,
        }
    }
...
pub(super) fn resolve_component_style(
    ...
    if let Some(width) = legacy.prose_max_width {
        style = style.with_prose_max_width(width);
    }
    style
}
...
#[cfg(test)]
mod tests {
    use gpui::{AppContext as _, Modifiers, Styled as _, point, px};
    use std::sync::{
        Arc,
    ...
    impl Render for HoverLink {
        fn render(&mut self, _: &mut gpui::Window, _: &mut Context<Self>) -> impl IntoElement {
            div()
                .size_full()
                .text_size(px(14.))
                .child(super::TextView::markdown(
                    "hover-link",
    ...
    fn markdown_link_hover_shows_tooltip(cx: &mut TestAppContext) {
        ...
        let (_, cx) = cx.add_window_view(|window, cx| {
            let content = cx.new(|_| HoverLink);
