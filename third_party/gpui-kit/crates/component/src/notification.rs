...
impl NotificationList {
    ...
    pub fn push(
        ...
        self.notifications.push(
            ...
            notification,
            ToastOptions {
                timeout: autohide.then_some(autohide_after),
            },
        ...
        let stacks = groups.into_iter().map(|(anchor, stack_id, items)| {
            ...
            items
                ...
                .fold(
                    ...
                    |stack, (id, item)| {
                        let content = if matches!(anchor, Anchor::TopCenter | Anchor::BottomCenter)
                        {
                            div().flex().justify_center().child(item).into_any_element()
                        } else {
                            item.into_any_element()
                        };
                        stack.item(format!("{id:?}"), content)
                    },
                )
                .placement(anchor)
                ...
                .map(|this| match anchor {
                    Anchor::TopLeft => this.top(margins.top).left(margins.left).into_any_element(),
...
mod tests {
    ...
    fn custom_autohide_duration_is_honored(cx: &mut TestAppContext) {
        ...
        // Base starts the configurable visible interval after the entrance
        // phase, and unmounts only after the exit phase completes.
        cx.background_executor
            .advance_clock(NOTIFICATION_TRANSITION_DURATION);
        cx.run_until_parked();
        ...
        cx.background_executor
            .advance_clock(Duration::from_millis(51));
        cx.run_until_parked();
        cx.background_executor
            .advance_clock(NOTIFICATION_EXIT_DURATION);
        cx.run_until_parked();
