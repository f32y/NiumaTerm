...
impl RenderOnce for Tab {
    fn render(self, _: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        if !tab_bar_prefix {
            if self.ix == 0 && self.variant == TabVariant::Tab && self.top_corner_radius.is_none() {
        ...
        if self.top_corner_radius.is_some() && self.variant == TabVariant::Tab {
            normal_style.borders.top = px(1.);
            selected_style.borders.top = px(1.);
            disabled_style.borders.top = px(1.);
            hover_style.borders.top = px(1.);
        }
        ...
        self.base
            ...
            .rounded(radius)
            .when_some(self.top_corner_radius, |this, radius| {
                this.rounded_t(radius).rounded_b(px(0.))
            })
            ...
            .hover(|this| {
                ...
                let this = this
                    ...
                    .rounded(radius);
                if let Some(radius) = self.top_corner_radius {
                    this.rounded_t(radius).rounded_b(px(0.))
                } else {
                    this
                }
