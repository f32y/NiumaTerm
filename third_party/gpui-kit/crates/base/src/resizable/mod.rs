...
impl ResizableState {
    ...
    /// Split the panel at `ix` in half, inserting the new half before or
    /// after it (used by split-pane creation: the new pane takes exactly half
    /// the split pane's extent, leaving siblings untouched). No-op when the
    /// half would fall below [`PANEL_MIN_SIZE`] or `ix` is out of range.
    pub fn split_panel(&mut self, ix: usize, before: bool, cx: &mut Context<Self>) {
        if ix >= self.sizes.len() {
            return;
        }
        let half = px(self.sizes[ix].as_f32() / 2.0);
        if half < PANEL_MIN_SIZE {
            return;
        }
        self.sizes[ix] = half;
        self.panels[ix].size = Some(half);
        let insert_at = if before { ix } else { ix + 1 };
        self.panels.insert(
            insert_at,
            ResizablePanelState {
                size: Some(half),
        ...
        self.sizes.insert(insert_at, half);
        cx.notify();
    }

    /// Set all panel sizes as fractions of the current container size (layout
    /// restore). No-op until the container has real bounds, or when `ratios`
    /// does not match the panel count.
    pub fn set_ratios(&mut self, ratios: &[f32], cx: &mut Context<Self>) {
        let container = self.container_size();
        if container.is_zero() || ratios.len() != self.panels.len() {
            return;
        }
        for (i, ratio) in ratios.iter().enumerate() {
            let size = container * *ratio;
            self.sizes[i] = size;
    ...
    /// True once the group has been laid out (container bounds known), so
    /// [`Self::set_ratios`] would take effect.
    pub fn has_bounds(&self) -> bool {
        !self.container_size().is_zero()
    }
...
mod tests {
    ...
    #[gpui::test]
    fn split_and_restore_preserve_sibling_sizes(cx: &mut TestAppContext) {
        let state = cx.update(|cx| cx.new(|_| ResizableState::default()));
        state.update(cx, |state, cx| {
            assert!(!state.has_bounds());
            state.bounds.size = size(px(600.), px(100.));
            state.panels = vec![Default::default(), Default::default()];
            state.sizes = vec![px(400.), px(200.)];
            state.split_panel(0, false, cx);
            assert_eq!(state.sizes(), &vec![px(200.); 3]);
            state.set_ratios(&[0.5, 0.25, 0.25], cx);
            assert_eq!(state.sizes(), &vec![px(300.), px(150.), px(150.)]);
            state.split_panel(1, true, cx);
            assert_eq!(state.sizes().len(), 3, "too-small panels cannot split");
            state.split_panel(20, true, cx);
            state.set_ratios(&[1.], cx);
            assert_eq!(state.sizes(), &vec![px(300.), px(150.), px(150.)]);
        });
    }
