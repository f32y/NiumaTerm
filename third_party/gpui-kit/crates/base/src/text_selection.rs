...
#[cfg(test)]
#[path = "text_selection_bounds_tests.rs"]
mod bounds_tests;
...
fn selected_line_bounds(
    ...
    let on_line = |p: Point<Pixels>| p.y >= line.top() && p.y < line.bottom();
...
type SelectionBoundsCallback = Rc<dyn Fn(&[Bounds<Pixels>], &App) -> Option<Bounds<Pixels>>>;
...
struct SelectableTextState {
    selection_bounds: Option<SelectionBoundsCallback>,
...
impl TextSelectionHandle {
    ...
    /// Resolves visible bounds for a renderer-local selection such as select-all.
    pub fn selected_bounds_with(
        &self,
        callback: impl Fn(&[Bounds<Pixels>], &App) -> Option<Bounds<Pixels>> + 'static,
        cx: &mut App,
    ) {
        self.0.update(cx, |state, _| {
            state.selection_bounds = Some(Rc::new(callback))
        });
    }
...
impl TextSelection {
    ...
    pub fn selected_text_bounds(window: &mut Window, cx: &mut App) -> Option<Bounds<Pixels>> {
        let state = live_text_selection_state(window, cx)?;
        let state = state.read(cx);
        let mut result: Option<Bounds<Pixels>> = None;
        for entry in state.participants.values() {
            if entry.registration.scope != state.active_scope {
                continue;
            }
            let Some(participant) = entry.participant.upgrade() else {
                continue;
            };
            let participant = participant.read(cx);
            let lines = &entry.registration.text_bounds;
            let bounds = if participant.local_selection {
                if let Some(callback) = &participant.selection_bounds {
                    callback(lines, cx)
                } else {
                    lines.iter().copied().reduce(|a, b| a.union(&b))
                }
            } else if let Some(points) = participant.snapshot.and_then(|s| s.window_points()) {
                lines
                    .iter()
                    .filter_map(|line| {
                        selected_line_bounds(*line, points.anchor(), points.cursor())
                    })
            ...
            if let Some(bounds) = bounds {
                result = Some(result.map_or(bounds, |current| current.union(&bounds)));
            }
        }
        result
    }
