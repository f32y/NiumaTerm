...
impl TextViewStyle {
    /// Constrain prose while leaving code blocks and tables full-width.
    pub fn with_prose_max_width(mut self, width: Rems) -> Self {
        self.prose_max_width = Some(width);
        self
    }

    /// The optional width limit for prose blocks.
    pub fn prose_max_width(&self) -> Option<Rems> {
