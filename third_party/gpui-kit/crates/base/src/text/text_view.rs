...
pub struct TextViewDefaults {
    link_tooltip: Option<
        Arc<dyn Fn(Option<SharedString>, Bounds<Pixels>, &mut Window, &mut App) + Send + Sync>,
    >,
...
impl TextViewDefaults {
    ...
    /// Installs an application-owned link tooltip presenter. `None` hides it.
    pub fn with_link_tooltip(
        mut self,
        handler: impl Fn(Option<SharedString>, Bounds<Pixels>, &mut Window, &mut App)
        + Send
        + Sync
        + 'static,
    ) -> Self {
        self.link_tooltip = Some(Arc::new(handler));
        self
    }

    pub(crate) fn request_link_tooltip(
        &self,
        ...
        if let Some(handler) = &self.link_tooltip {
            handler(url, bounds, window, cx);
        }
    }
...
impl TextView {
    ...
    /// Create a plain-text view with literal markup and hard line breaks.
