...
impl InputBaseState<EditorMode> {
    pub(crate) fn handle_completion_trigger(
        ...
        self.schedule_inline_completion(range, new_text, window, cx);
    ...
    pub(crate) fn schedule_inline_completion(
        ...
        // Typing forward along the ghost text keeps it visible across the
        // debounce (trimming the typed prefix); clearing unconditionally
        // blanks the suggestion for a few frames per keystroke, which reads
        // as flicker.
        let kept = self
            .extras
            .inline_completion
            .item
            .as_ref()
            .and_then(|item| trim_ghost_text(&item.insert_text, range, new_text));
        if let Some(rest) = kept {
            if let Some(item) = self.extras.inline_completion.item.as_mut() {
                item.insert_text = rest;
            }
            cx.notify();
        } else {
...
/// The ghost text still valid after replacing `replaced` with `new_text`:
/// a pure insertion of the ghost's own prefix trims it; any other edit
/// invalidates it (`None`).
fn trim_ghost_text(ghost: &str, replaced: &Range<usize>, new_text: &str) -> Option<String> {
    if !replaced.is_empty() || new_text.is_empty() {
        return None;
    }
    let rest = ghost.strip_prefix(new_text)?;
    (!rest.is_empty()).then(|| rest.to_string())
}

#[cfg(test)]
mod tests {
    use super::trim_ghost_text;

    #[test]
    fn trim_ghost_text_tracks_forward_typing() {
        // Typing the ghost's next chars trims them off.
        assert_eq!(trim_ghost_text("witch", &(5..5), "w"), Some("itch".into()));
        assert_eq!(trim_ghost_text("witch", &(5..5), "wit"), Some("ch".into()));
        // Typing the whole ghost leaves nothing to show.
        assert_eq!(trim_ghost_text("witch", &(5..5), "witch"), None);
        // Divergent character invalidates.
        assert_eq!(trim_ghost_text("witch", &(5..5), "x"), None);
        // Deletion (empty insert) invalidates.
        assert_eq!(trim_ghost_text("witch", &(4..5), ""), None);
        // Replacement of a selection invalidates.
        assert_eq!(trim_ghost_text("witch", &(3..5), "w"), None);
    }
}
