...
impl<M: InputModeKind> InputBaseState<M> {
    ...
    pub(super) fn delete(&mut self, _: &Delete, window: &mut Window, cx: &mut Context<Self>) {
        let intent = if self.selected_range.is_empty() {
            if let Some(link) = self.link_at(self.cursor()) {
                self.selected_range = link.into();
                EditIntent::Atomic
            } else {
                self.select_to(self.next_boundary(self.cursor()), cx);
...
mod tests {
    ...
    #[gpui::test]
    fn links_delete_atomically_and_silent_replacement_clears_ranges(cx: &mut TestAppContext) {
        let input_view = InputView::build(cx, |state| state.cursor_block(true));
        let mut cx = VisualTestContext::from_window(input_view.window_handle.into(), cx);
        let input = input_view.input;
        cx.update(|window, cx| {
            input.update(cx, |state, cx| {
                ...
                state.undo(&Undo, window, cx);
                assert_eq!(state.value(), "a link z");
                state.set_selected_range(2..2, cx);
                state.delete(&Delete, window, cx);
                assert_eq!(state.value(), "a  z");
                state.set_value("replacement", window, cx);
                assert!(state.links.is_empty());
                assert!(state.cursor_block);
            })
        });
    }
