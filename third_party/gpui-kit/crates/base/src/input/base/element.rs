...
impl<M: InputModeKind> TextElement<M> {
    ...
    fn layout_cursor(
        ...
        let cursor_char_width = if state.cursor_block {
            visible_buffer_lines
                .iter()
                .position(|&row| row == cursor_row)
                .and_then(|visible_index| {
                    let line = &lines[visible_index];
                    let line_start = last_layout.visible_line_byte_offsets[visible_index];
                    line.char_width_at(cursor.saturating_sub(line_start))
                })
        } else {
            None
        };
...
impl<M: InputModeKind> Element for TextElement<M> {
    ...
    fn prepaint(
        ...
        let runs = if let (false, Some(highlight_styles)) = (is_empty, highlight_styles) {
            ...
            let base_runs = if !is_empty && !state.masked && !state.links.is_empty() {
                link_runs(
                    &state.links,
                    &run,
                    dim(state.editor_style.link),
                    display_text.len(),
                )
            } else {
                vec![run]
