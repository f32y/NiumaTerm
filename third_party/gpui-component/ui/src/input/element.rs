...
use crate::{ActiveTheme as _, Colorize, IconName, Root, Selectable, Sizable as _};
...
impl TextElement {
    ...
    fn layout_cursor(
        ...
        // Advance of the character under the block caret, if there is one.
        let mut cursor_char_width = None;
            ...
            if let Some(line) = line_layout {
                if cursor_pos.is_none() {
                        ...
                        line.position_for_index(offset, last_layout, state.cursor_line_end_affinity)
                    {
                        ...
                        if state.cursor_block {
                            cursor_char_width = line.char_width_at(offset);
                        }
                    }
                }
            ...
            (cursor_pos, cursor_start, cursor_end)
        {
            ...
            // cursor bounds
            let cursor_height = if state.cursor_block {
                line_height
            } else {
                let factor = match state.size {
                    crate::Size::Large => 1.,
            ...
            // Block caret spans exactly the character under it. At end of
            // line there is no glyph, so fall back to the shaped advance of
            // "n" (a typical character width; shape_line results are cached).
            let cursor_width = if state.cursor_block {
                cursor_char_width
                    .filter(|w| *w > px(0.))
                    .unwrap_or_else(|| {
    ...
    fn layout_line_numbers(
        ...
        let mut line_number_width = if state.mode.line_number() {
            ...
            // Local change from upstream (which kept a LINE_NUMBER_RIGHT_MARGIN
            // inset for multi-line code editors without line numbers): a code
            // editor with line numbers off starts text flush-left, like a plain
            // textarea. The prompt dock relies on this.
            px(0.)
        };
...
impl Element for TextElement {
    ...
    fn request_layout(
        ...
        if state.mode.is_multi_line() {
            style.flex_grow = 1.0;
            style.size.height = relative(1.).into();
            if state.mode.is_content_sized() {
                // Grow the height to match the wrapped row count, capped at max rows.
                // Covers AutoGrow and a code editor that opted into auto-grow.
    ...
    fn paint(
        ...
        if focused {
            if let Some(first_line) = &prepaint.ghost_first_line {
                ...
                {
                    let cursor_right = cursor_bounds.origin.x + cursor_bounds.size.width;
                    // A block caret occupies the cell the completion's first
                    // character will land on, so the ghost starts at the caret
                    // cell (first character inside the block) and accepting
                    // does not shift the text; a beam caret only precedes that
                    // cell, so the ghost starts past it.
                    let first_line_x = if self.state.read(cx).cursor_block {
                    ...
                    // Paint background to cover any existing text, starting
                    // past the caret so a block caret stays visible.
                    let bg_width =
                        (first_line.width + px(4.) - (cursor_right - first_line_x)).max(px(0.));
                    let bg_bounds = Bounds::new(
                        point(cursor_right, cursor_row_y),
                        size(bg_width, line_height),
                    );
