...
impl LineLayout {
    ...
    /// Advance width of the character at the given local byte offset,
    /// or None if the offset is at (or past) the end of the line.
    pub(crate) fn char_width_at(&self, offset: usize) -> Option<Pixels> {
        let mut acc_len = 0;
        for line in self.wrapped_lines.iter() {
            if offset < acc_len + line.len {
                let rel = offset - acc_len;
                let c = line.text.get(rel..)?.chars().next()?;
                return Some(line.x_for_index(rel + c.len_utf8()) - line.x_for_index(rel));
            }
            acc_len += line.len;
        }
        None
