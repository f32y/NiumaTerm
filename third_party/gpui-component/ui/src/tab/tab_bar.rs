...
pub struct TabBar {
    ...
    inline_suffix: Option<AnyElement>,
...
impl TabBar {
    ...
    /// Set an element rendered inside the tabs row, right after the last tab
    /// (it scrolls with the tabs), unlike `suffix` which is pinned to the
    /// right edge of the bar.
    pub fn inline_suffix(mut self, suffix: impl IntoElement) -> Self {
        self.inline_suffix = Some(suffix.into_any_element());
        self
    }
...
impl RenderOnce for TabBar {
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        self.base
            ...
            .child(
                // `min_w_0` breaks flexbox min-width:auto propagation, so the
                // tabs region shrinks to the bar's viewport instead of being
                // blown up to its content width; without it the scroll handle
                // sees nothing overflowing and `scroll_to_item` never scrolls.
                h_flex()
                    .id("tabs")
                    .flex_1()
                    .min_w_0()
                    ...
                    .child(
                        h_flex()
                            ...
                            .when_some(self.inline_suffix, |this, inline_suffix| {
                                this.child(inline_suffix)
                            })
